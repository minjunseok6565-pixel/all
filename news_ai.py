...
def _get_current_date() -> date:
    ...
    cur = get_current_date() or league_context.get("season_start")
    if not cur:
        raise RuntimeError("In-game current_date is required for news generation (OS clock disabled).")
    try:
        return date.fromisoformat(str(cur)[:10])
    except Exception as exc:
        raise ValueError(f"Invalid in-game date ISO: {cur!r}") from exc
